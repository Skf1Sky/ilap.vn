...
import { Link } from 'react-router-dom'; // 👈 QUAN TRỌNG: Phải import cái này
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-xl hover:border-primary transition duration-300 group h-full flex flex-col">
      <div className="relative h-[240px] bg-gray-100 overflow-hidden">
        ...
        {/* 👇 SỬA CHỖ NÀY: Dùng Link thay cho thẻ a */}
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h5 className="font-bold text-lg mb-1 group-hover:text-primary transition line-clamp-1">
            {/* 👇 SỬA CHỖ NÀY LUÔN */}
        ...
        <div className="mt-auto flex justify-between items-center pt-3">
          ...
          {/* 👇 SỬA NÚT CHI TIẾT */}
          <Link to={`/product/${product.id}`} className="px-4 py-1.5 bg-blue-50 text-primary text-xs font-bold rounded hover:bg-primary hover:text-white transition">
            Chi tiết
          </Link>
        </div>
      </div>
